...
a.按照《TDengine安装指南》第2章 安装前准备 配置所有节点。

b.配置操作端到各节点的SSH互信。

c.准备TDengine安装包、部署脚本、节点配置文件
...
```shell
##/tmp/tdengine/update.sh
sfile=/tmp/tdengine/TDengine-enterprise-server-2.2.1.4-Linux-x64.tar.gz
dir=/tmp/tdengine_soft
mkdir -p $dir
...
pmes()
{
    ...
    NC='\033[0m'
    echo
    echo -e "${wab} $(date +'%Y-%m-%d %H:%M:%S') ${1} ${NC}"
    echo
}
...
cd TDengine*
echo
pmes "Install TDengine ......"
bash install.sh -e no  1>/dev/null 2>/dev/null
...
do
    if [ -n `pidof taosd` ]
    then
        pmes "TDengine Start successfully!"
        break;
    else
        systemctl start taosd
    fi
...
```shell
scp -r /tmp/tdengine root@c0-70:/tmp/
scp -r /tmp/tdengine  root@c0-71:/tmp/
scp -r /tmp/tdengine  root@c0-72:/tmp/
scp -r /tmp/tdengine  root@c0-75:/tmp/
scp -r /tmp/tdengine  root@c0-76:/tmp/
scp -r /tmp/tdengine  root@c0-77:/tmp/
...
```shell
ssh root@c0-70 "bash /tmp/tdengine/update.sh"
ssh root@c0-71 "bash /tmp/tdengine/udpate.sh"
ssh root@c0-72 "bash /tmp/tdengine/update.sh"
ssh root@c0-75 "bash /tmp/tdengine/update.sh"
ssh root@c0-76 "bash /tmp/tdengine/update.sh"
ssh root@c0-77 "bash /tmp/tdengine/update.sh"
```
...
以下配置文件以TDengine-enterprise-server-2.2.1.4-Linux-x64.tar.gz为例，使用时请修改为实际版本。
